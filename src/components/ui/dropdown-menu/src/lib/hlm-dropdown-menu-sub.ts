...
export class HlmDropdownMenuSub {
  ...
  private setSideWithDarkMagic() {
    ...
    setTimeout(() => {
      // our menu trigger directive leaves the last position used immediately after opening.
      // we can access it here and determine the correct side.
      type HostWithParentTrigger = {
        _parentTrigger?: {
          _spartanLastPosition?: ConnectedPosition;
        };
      };

      const ps = (this._host as unknown as HostWithParentTrigger)._parentTrigger
