...
export function classes(computed: () => ClassValue[] | string, options: ClassesOptions = {}) {
  runInInjectionContext(options.injector ?? inject(Injector), () => {
    ...
    const manager =
      elementClassManagers.get(element) ??
      (() => {
        // Initialize base classes from variation (host attribute 'class')
        const initialBaseClasses = new Set<string>();

        if (baseClasses) {
          for (const cls of toClassList(baseClasses)) {
            initialBaseClasses.add(cls);
          }
        }

        const createdManager: ElementClassManager = {
          element,
          sources: new Map(),
        ...
        elementClassManagers.set(element, createdManager);
        ...
        if (isPlatformBrowser(platformId)) {
          createdManager.previousTransition = element.style.getPropertyValue('transition');
          createdManager.previousTransitionPriority =
            element.style.getPropertyPriority('transition');
          element.style.setProperty('transition', 'none', 'important');
          createdManager.transitionsSuppressed = true;
        }
...
function setupGlobalObserver(platformId: unknown): void {
  if (isPlatformBrowser(platformId as object) && !globalObserver) {
